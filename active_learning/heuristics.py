...
def _get_labels_to_indices(labels):
    ...
    For examples:
        >>> LABELS = ["lb1", "lb2", "lb2", "lb3"]
        >>> _get_labels_to_indices(LABELS)

    Args:
        labels ( List[str or int]): List of labels
    Returns:
        labels_to_indices (defaultdict): Dictionary that maps unique label to indices where the label occured in the
        occured in the original data
    """
...
class Heuristic(ABC):
    ...
    @staticmethod
    @abstractmethod
    def rank_2d(confidence_2d: List[List[float]]) -> List[int]:
        raise NotImplementedError("Subclasses must be implement this method.")

    @staticmethod
    @abstractmethod
    def rank_3d(confidence_3d: List[List[List[float]]]) -> List[int]:
        raise NotImplementedError("Subclasses must be implement this method.")
    ...
    def _convert_to_sample_ranks(ordered_sample_indices: List[int]):
        """
        Args:
            ordered_sample_indices (List[int]). List of indices corresponding to value ordered
            from least to greatest.
        Returns:
            sample_ranks (List[int]): List where the value at each index is the rank of corresponding sample.
        """
    ...
    @staticmethod
    def ordered_indices_list_to_final_rank(ordered_sample_indices_list: List[List[int]]):
...
class LeastConfidenceSampling(ABC):
    ...
    def rank_2d(confidences_2d: List[List[float]]) -> List[int]:
        ...
        the elements from the lowest confidence to the highest confidence.
...
class MarginSampling(ABC):
    @staticmethod
    def rank_2d(confidences_2d: List[List[float]]) -> List[int]:
        """Calculates the "margin" or difference between the highest and second the highest
